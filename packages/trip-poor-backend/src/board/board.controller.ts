...
export class BoardController {
  ...
  @ApiOperation({ summary: '모든 게시물 조회', description: '모든 게시물을 조회합니다.' })
  ...
  @ApiOperation({ summary: '특정 게시물 조회', description: '특정 게시물을 조회합니다.' })
  @ApiResponse({ status: 200, description: '성공', type: BoardEntity })
  @Get(':id')
  async getBoard(@Req() req: Request, @Res() res: Response, @Param('id') id: number) {
    return res.json(await this.boardService.getBoard(id));
  }

  @ApiOperation({ summary: '게시판 생성', description: '게시판을 생성합니다.' })
  @ApiBody({ type: CreateBoardDto, description: '게시판 생성 정보', required: true })
  @ApiResponse({ status: 200, description: '성공', type: BoardEntity })
  @Post()
